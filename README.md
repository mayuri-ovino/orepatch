...
A Fabric mod for Minecraft 26.1.2 that finds ores hidden behind only a
...
This project targets **Minecraft 26.1.2**, which is the first unobfuscated
release of the game. That changes the toolchain in a few important ways
versus older Fabric mods you may have seen:

- There's no Yarn mappings anymore — code is written directly against
  Mojang's own (now-public) class names, e.g. `ServerLevel` instead of
  `ServerWorld`, `Level` instead of `World`, `Identifier` instead of
  `ResourceLocation`.
- The buildscript uses the plain `jar`/`implementation` Gradle tasks
  ...
  step anymore.
- You'll need **Java 25** and **Fabric Loom 1.15+**.
...
1. Install [Fabric Loader](https://fabricmc.net/use/) 0.18.0 or newer for
   Minecraft 26.1.2.
2. Download/build **Fabric API** for 26.1.2 and drop it in your `mods`
